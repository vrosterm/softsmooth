...
from SU25_BASECODE.train_save_smooth import epoch, epoch_adversarial, pgd_linf, evaluate_clean, evaluate_under_attack, phi_inverse
...
if __name__ == '__main__': 
    if not os.path.exists("models/LWRS_model_test.pt"):
        for epoch_num in range(10):
            print(f"Epoch {epoch_num+1}:")
            train_err, train_loss = epoch(train_loader, LWRS_model, opt_LWRS)
            test_err, test_loss = epoch(test_loader, LWRS_model, opt_LWRS)
            adv_err, adv_loss = epoch_adversarial(test_loader, LWRS_model, pgd_linf, opt_LWRS, epsilon, alpha, num_iter)
        ...
        torch.save(LWRS_model.state_dict(), "models/LWRS_model_test.pt")

    # Load saved LWRS model
    LWRS_model.load_state_dict(torch.load("models/LWRS_model_test.pt", map_location=device))
